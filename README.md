...
### 📅 每日签到（v0.0.4.0）
- 玩家 `player_basic_info.last_checkin_date` 与服务器本地日期（YYYY-MM-DD）不同即可签到
- 主菜单 **每日签到**：发放配置存款 + 按权重不放回抽取的随机物品奖励
- 配置：`CHECKIN_DAILY_MONEY`、`CHECKIN_REWARD_PICK_COUNT`、`CHECKIN_REWARD_LIST`（JSON 数组，每项 `[物品ID, 数量, 权重]`）；**OP 面板 → 签到配置** 可视化编辑
